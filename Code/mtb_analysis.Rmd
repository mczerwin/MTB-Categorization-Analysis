...
labels <- as.factor(unlist(lapply(new_mtb_data$label, remap)))
n <- length(labels)
...
trainSVM <- function(x){
  xtest <- mtb_standard_scaled[x,]
  xtrain <- mtb_standard_scaled[-x,]
  ytest <- labels[x]
  ytrain <- labels[-x]
...
y_pred <- predict(clf, Xtest)

sum(table(y_pred, y_pred))
