...
mtb_data <- read_excel(here('Data/mtb_stats.xlsx'), 'Sheet1')
...
- `fork_travel` has a correlation above .95 with the variables: `r mtb_correlation_df %>% select(variable, fork_travel) %>% filter((fork_travel > .95) | (fork_travel < -.95)) %>% select(variable) %>% as.character()`. Some of these make sense; for example, `rear_travel` *should* be highly correlated with `fork_travel`. Some of the other variables would require a bit more analysis, but are good to note.
...
## 2. PCA
Next, we'll apply PCA to our dataset.
