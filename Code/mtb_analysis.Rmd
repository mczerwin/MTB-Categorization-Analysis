...
for (c in 1:ncol(mtb_data)){
  if (is.numeric(unlist(mtb_data[,c]))){
    print(colnames(mtb_data)[c])
    mtb_data[is.na(mtb_data[,c]), c] <- mean(unlist(mtb_data[,c]), na.rm=TRUE)  
  }
}

# TODO get average bikes by setting  to work above
mtb_no_null <- mtb_data %>% 
                select(-c(price, url, build_type, image, setting)) %>% 
                select_if(is.numeric) %>% 
...
#TODO keep row names on the data, but not as an actual column
...
cols <- unlist(lapply(mtb_data$label, colorgroups))


plot(pca_2_scaled, col=cols)
points(pca_km_scaled$centers, col='black', pch='x')

#TODO let's look at this bottom right cluster
